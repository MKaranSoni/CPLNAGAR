...
export default function ComplaintTracking() {
  ...
  const loadComplaints = async () => {
    try {
      ...
      const normalizedData = rawData.map(c => ({
        ...c,
        locationName: c.locationName || `${c.latitude?.toFixed(2) || 'N/A'}, ${c.longitude?.toFixed(2) || 'N/A'}`,
        photoUrl: c.imageUrl || c.photoUrl || "https://images.unsplash.com/photo-1618220179428-22790b461013?w=600&auto=format&fit=crop&q=80",
        priority: c.severity || c.priority || "Medium",
        assignedVolunteer: c.assignedVolunteer || "Unassigned"
      }));
      setComplaints(normalizedData);
      if (normalizedData.length > 0) {
        setSelectedCompId(normalizedData[0].id);
      }
    } catch (err) {
  ...
  const updateComplaintStatus = async (id, status, msg) => {
    try {
      await backendUpdateComplaintStatus(id, status);
      showToast(msg || "Status updated successfully!", "success");
      loadComplaints();
    } catch (err) {
          ...
          <div className="lg:col-span-2 space-y-6">
                ...
                {stages.map((stg, idx) => {
                  ...
                  return (
                    <div key={stg.label} className="flex sm:flex-col items-center gap-3 relative z-10 w-full sm:w-auto text-left sm:text-center">
                      ...
                      <div>
                        <p className={`text-[10px] font-bold ${isDone ? 'text-slate-800 dark:text-slate-100' : 'text-slate-400'} ${isCurrent ? 'text-brand-violet font-extrabold' : ''}`}>{stg.label}</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-6 flex flex-col">
                ...
                {/* QR Auditor */}
                <div className="p-5 rounded-3xl glass-panel-glow border-brand-violet/20 bg-white/70 dark:bg-slate-900/70 text-center space-y-4 shadow-soft">
                  <div className="flex items-center justify-center gap-2">
                    <QrCode className="w-5 h-5 text-brand-violet animate-pulse" />
                    <span className="text-xs font-bold font-outfit text-slate-800 dark:text-slate-100">Physical On-Site QR Verification</span>
                  </div>

                  <div className="w-24 h-24 bg-white dark:bg-slate-900 p-2 rounded-xl mx-auto flex items-center justify-center relative group border border-purple-100 shadow-soft">
              ...
              <div className="p-5 rounded-3xl glass-panel flex flex-col justify-between h-[480px] bg-white/70 dark:bg-slate-900/70 shadow-soft">
                ...
                <div className="flex items-center justify-between border-b border-purple-100 pb-3 mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h4 className="text-xs font-bold text-slate-800 dark:text-slate-100">Municipal Chat log</h4>
                ...
                <form onSubmit={handleSendMessage} className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-white dark:bg-slate-900 border border-purple-100 text-slate-800 dark:text-slate-100 px-3.5 py-2.5 rounded-xl text-xs focus:outline-none focus:border-brand-violet/50 shadow-soft"
                  />
                  <button
                    type="submit"
                    className="p-2.5 rounded-xl bg-white dark:bg-slate-900 border border-purple-100 hover:bg-purple-50 text-brand-violet hover:text-brand-purple transition-colors shadow-soft"
                  >
